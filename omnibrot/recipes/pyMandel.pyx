...
def pyMandel(center, ranges, shape, maxIter, flavor="default"):
    data = np.ndarray(shape, dtype=int)
    ...
    if flavor == "default":
        ...
        mandel(&view[0, 0], cCFPHPN(center), cCFPHPN(ranges), ComplexNumber[int](shape[0], shape[1]), <long> maxIter)
    elif flavor == "fast":
        meta["algorithm"] = "double"
    elif flavor == "relative":
        meta["algorithm"] = "relative(FPHPN, double)"
        ranges = complex(ranges)
        mandel(&view[0, 0], cCFPHPN(center), ComplexNumber[double](ranges.real, ranges.imag), ComplexNumber[int](shape[0], shape[1]), <long> maxIter)
    else:
